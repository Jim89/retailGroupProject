...
# Step 1 - log the price variables for semi-log model ---------------------------
...
carte_elasts <- extract_elasticities(carte_fit)*mean(heavy$carte_noire_price)
douwe_elasts <- extract_elasticities(douwe_fit)*mean(heavy$douwe_egbert_price)
kenco_elasts <- extract_elasticities(kenco_fit)*mean(heavy$kenco_price)
nesca_elasts <- extract_elasticities(nesca_fit)*mean(heavy$nescafe_price)
other_elasts <- extract_elasticities(other_fit)*mean(heavy$other_brands_price)
super_elasts <- extract_elasticities(super_fit)*mean(heavy$supermarket_own_price)
...
semi_log_heavy_elasticities_clean <- heavy_elasticities * heavy_signifs

# Step 5 - Clean up and garbage collect ----------------------------------------
objects <- ls()
idx <- grep("coffee_clean||heavy|heavy|heavy_elasticities_clean|normalise", objects)
objects <- objects[-idx]

rm(list = objects)
rm(objects, idx, heavy_elasticities, heavy_signifs, heavy)
gc()
