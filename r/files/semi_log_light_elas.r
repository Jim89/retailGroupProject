...
carte <- lm(log(carte_noire_sales) ~ ., data = light)
douwe <- lm(log(douwe_egbert_sales) ~ ., data = light)
kenco <- lm(log(kenco_sales) ~ ., data = light)
nesca <- lm(log(nescafe_sales) ~ ., data =light)
other <- lm(log(other_brands_sales) ~ ., data = light)
super <- lm(log(supermarket_own_sales) ~ ., data = light)
...
carte_elasts <- extract_elasticities(carte_fit)*mean(light$carte_noire_price)
douwe_elasts <- extract_elasticities(douwe_fit)*mean(light$douwe_egbert_price)
kenco_elasts <- extract_elasticities(kenco_fit)*mean(light$kenco_price)
nesca_elasts <- extract_elasticities(nesca_fit)*mean(light$nescafe_price)
other_elasts <- extract_elasticities(other_fit)*mean(light$other_brands_price)
super_elasts <- extract_elasticities(super_fit)*mean(light$supermarket_own_price)
...
semi_log_light_elasticities_clean <- light_elasticities * light_signifs

# Step 5 - Clean up and garbage collect ----------------------------------------
objects <- ls()
idx <- grep("coffee_clean||light|light|light_elasticities_clean|normalise", objects)
objects <- objects[-idx]

rm(list = objects)
rm(objects, idx, light_elasticities, light_signifs, light)
gc()
